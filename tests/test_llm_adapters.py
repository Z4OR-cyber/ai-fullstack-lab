...
class TestOpenAIResponseParsing:
    ...
    # ── Ollama / local model content-embedded tool call fallback ──

    def test_parse_content_tool_call_single(self):
        """Ollama emits {name, arguments} JSON in content."""
        adapter = OpenAIAdapter(api_key="sk-test")
        data = {
            "choices": [{
                "message": {
        ...
        assert tool_calls[0].id  # auto-generated id

    def test_parse_content_tool_call_string_args(self):
        """arguments may be a JSON string rather than object."""
        adapter = OpenAIAdapter(api_key="sk-test")
        data = {
            "choices": [{
                "message": {
                    "role": "assistant",
                    "content": '{"name": "calc", "arguments": "{\\"expr\\": \\"1+1\\"}"}',
                }
            }],
        }
        tool_calls = adapter._parse_tool_calls(data)
        ...
        assert tool_calls[0].arguments == {"expr": "1+1"}

    def test_parse_content_tool_call_multiline(self):
    ...
    def test_parse_content_tool_call_code_fence(self):
        """Tool call wrapped in ```json fence."""
        adapter = OpenAIAdapter(api_key="sk-test")
        data = {
            "choices": [{
                "message": {
    ...
    def test_parse_content_tool_call_ignores_plain_text(self):
        """Normal content without name field is not mis-parsed."""
        adapter = OpenAIAdapter(api_key="sk-test")
        data = {
            "choices": [{
    ...
    def test_parse_standard_tool_calls_takes_priority(self):
        """If both tool_calls array and content exist, array wins."""
        adapter = OpenAIAdapter(api_key="sk-test")
        data = {
            "choices": [{
                "message": {
                    "role": "assistant",
                    "content": '{"name": "should_not_parse", "arguments": {}}',
                    "tool_calls": [{
                        "id": "call_real",
                        "type": "function",
        ...
        assert tool_calls[0].id == "call_real"

    def test_content_cleared_when_tool_call_parsed_from_content(self):
        """When tool call is extracted from content, content should be None."""
        import asyncio
        from unittest.mock import AsyncMock, MagicMock

        async def run():
            adapter = OpenAIAdapter(api_key="sk-test", model="test-model")
            mock_resp = MagicMock()
            ...
            # Patch _get_client to return our mock directly
